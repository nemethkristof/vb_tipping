    ...
    const fetchData = async () => {
      try {
        ...
        rawPredictions.forEach((prediction) => {
          if (!userScores[prediction.user]) userScores[prediction.user] = 0
        })
        ...
        const rankedLeaderboard = sortedScores.map((player, index, array) => {
          if (index > 0 && player.score < array[index - 1].score) currentRank = index + 1
    ...
    <Box sx={{ minHeight: '100vh', background: '#f5f5f5', py: 4 }}>
      <Container maxWidth="lg">
        <Box sx={{ mb: 4, textAlign: 'center' }}>
          <Typography variant="h1" sx={{ fontSize: isMobile ? '2rem' : '3rem', fontWeight: 800, color: 'primary.main', mb: 2, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
